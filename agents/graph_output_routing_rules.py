...
- For visual intent, call `load_skill("graph_planner")`, wait for the planner result, then call `load_skill("graph_writer")`; do not stop after planning. Never request planner and writer in the same tool-call batch. After `load_skill("graph_writer")`, the very next tool call must be `run_graph`."""
