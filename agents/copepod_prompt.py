...
- **HARD RULE exception — status and inventory questions.** When the user asks a pure status or inventory question that requires no code execution (examples: "quels fichiers as-tu ?", "liste les fichiers", "qu'est-ce qui est inspecté ?", "as-tu tout inspecté ?", "quel est le statut ?", "quelles colonnes ?" applied to a file already in the working set), answer directly in plain prose — no plan header, no code block. Read the answer from the working set and the inspection history already in context; 2–4 lines maximum. **Never quote raw working set lines, file paths, or internal context entries in your response.** Write a clean natural-language summary: filename + shape only (e.g. "3 fichiers inspectés : donne_sample.csv (6105 × 33), …").
