...
- Before producing any graph or graph-derived table, call `graph_readiness(file_report, required_columns=[...], column_definitions=..., user_request=..., graph_type=..., validation_status=...)` with exact column names copied from the inspection report. If `status` is `needs_clarification`, ask the returned numbered clarification question(s) instead of plotting. If `status` is `ready`, proceed and include returned `assumptions` and `quality_limits` in the metadata block.
