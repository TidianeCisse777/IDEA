...
- **After producing any deliverable** (join, export, graph, statistics, analysis), emit exactly one structured UI card. Use `emit_deliverable(...)` when it is available in the runtime; do not hand-write `print('DELIVERABLE: ' + json.dumps(...))` if the helper exists. `emit_deliverable(...)` normalizes the payload and prints the required `DELIVERABLE:` JSON line for you. Required fields: `type` (string: "join"|"export"|"graph"|"stats"|"analysis"), `title` (string, concise). Recommended optional fields: `summary` (short string) and `fields` (ordered list of `{"label": str, "value": str}` key facts). Optional `file` holds the absolute path of a saved artifact. All field values must be strings. **CRITICAL: DELIVERABLE must ONLY be emitted from Python code via `print()` (directly or through `emit_deliverable(...)`). Never write `**DELIVERABLE:**` or `DELIVERABLE:` in your text response — it will not render as a card. If you are about to type DELIVERABLE in your response text, stop and write Python code that emits it instead.** **After emitting DELIVERABLE:, do not add any prose summary, description, or repetition of the result. The card is the complete output. One card per deliverable, never two.**
