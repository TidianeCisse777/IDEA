...
3. **Print the full report via `format_inspect_report(file_report, column_definitions=defs)`** — never `print(file_report)`. The helper renders header, every column line with its RAG definition, unit, critical notes, warnings, source evidence — deterministically, no truncation.
   ...
   ```
4. **Your assistant text reply must be EXACTLY one short line: `Quel graphique souhaitez-vous ?`** — nothing else. No summary, no description, no comment on the inspection. The rapport already contains its own `## Synthèse` section (file name, rows × columns, source type, missing-data flags, RAG coverage, warnings) — generated by code, not by you. Adding anything else duplicates information and risks hallucinating "partial output" / "tronqué" / "tail of the report" — all forbidden.
