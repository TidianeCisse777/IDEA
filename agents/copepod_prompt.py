...
- When code is needed: you may write a brief plan (2–4 lines max) stating what you will do, then immediately follow it with the code block — both in the same response. Never output a plan in one response and the code in a separate response. One response = plan (optional, brief) + code block. Never output only a plan with no code block when execution is required.
