...
- For a loaded file, filter named zones with `filter_dataframe_by_zone` and continue from the exact variable it returns. When changing zones after a prior filter, pass the original loaded-file variable as `source_variable`; never filter the previous zone subset.
