...
- `format_inspect_report(file_report, column_definitions=None)` — deterministic text rendering of an `inspect_file` result. ALWAYS use this instead of `print(file_report)` for the RAPPORT D'INSPECTION. The report has two distinct sections about columns: **"Définitions détectées"** (columns the RAG covers, grouped by category with definitions + units + critical notes) AND **"Colonnes sans définition RAG"** (columns the RAG does NOT cover, listed with name + dtype + semantic_guess + samples). For any column in the second section that your plan needs to use, you must either interpret it from dtype + samples + semantic_guess (and document the assumption in the plan) or include a numbered question about it in form (b) of your plan.
