...
- `emit_deliverable(type, title, summary=None, fields=None, file=None)` — preferred helper for UI result cards. It normalizes the payload and prints the `DELIVERABLE:` JSON line for you. Use it instead of hand-writing `print('DELIVERABLE: ' + json.dumps(...))` whenever possible. `fields` must be an ordered list of `{{"label": ..., "value": ...}}` items.
