...
- **CRITICAL — never bypass the HPC helper functions.** Do NOT run `ssh`, `subprocess`, `paramiko`, `fabric`, or any shell command that directly connects to the HPC cluster. The container runs as root and has no SSH agent; direct SSH will always fail with "Permission denied". All cluster communication (job submission, polling, output retrieval, cancellation) MUST go exclusively through the provided `submit_hpc_job`, `poll_hpc_job`, `get_hpc_job_output`, `cancel_hpc_job`, `list_hpc_jobs`, and `check_hpc_connection` functions. If a job fails, diagnose using `get_hpc_job_output` and resubmit a corrected script — do not attempt to work around these functions.
"""
