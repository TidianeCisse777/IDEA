...
- **`_ContextMiddleware`** (agent construit via `create_agent`, LangChain 1.x) :
  ...
  - `AVAILABLE DATAFRAMES` projette l'inventaire checkpointé de façon uniforme : une entrée par table avec statut actif comme simple métadonnée, source, description, grain, schéma groupé par rôle et dtype, visibilité du schéma, clés, portée et lignée. Tous les DataFrames issus de fichiers sont toujours développés comme ancres sources canoniques; seul le nombre de fiches détaillées non-fichier est borné. Une référence `df_*` exacte dans la demande est promue dans ce sous-ensemble sans masquer les alternatives. Les anciens dérivés masqués n'entrent pas dans l'inventaire.
